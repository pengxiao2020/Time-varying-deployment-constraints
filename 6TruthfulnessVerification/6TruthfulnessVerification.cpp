...
void OTRAP(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& Car)
{
	...
	else		//执行OTRAP算法
	{
		...
		/*if (cts == 1076)
		{
			cout << "当前时间槽为 : cts = " << cts << ", 未获得分配的车辆集合 UnAllocatedCar 中的车辆信息，如下：" << endl;
			for (auto UACarit = UnAllocatedCar.begin(); UACarit != UnAllocatedCar.end(); UACarit++)
			{
				if (UACarit->id == 11499)
				{
					cout << "车辆id = " << UACarit->id << "  ";
					cout << "type = " << UACarit->type << "  ";
					cout << "InitLoc(m) = " << UACarit->InitLoc << "  ";
					cout << "v(m/s) = " << UACarit->v << "  ";
					cout << "at = " << UACarit->at << "  ";
					cout << "dt = " << UACarit->dt << "  ";
					cout << "et = " << UACarit->et << "  ";
					cout << "TRD = [" << UACarit->TRD->cpu << ", " << UACarit->TRD->gpu << ", " << UACarit->TRD->memory << ", " << UACarit->TRD->disk << "]  ";
					cout << "bid(coeffient) = " << UACarit->bid << "(" << UACarit->mu << ")" << endl;
				}
				else
				{
					cout << "运行 OTRAP_ALLOC 之前，id = 11499 的车辆未被添加进集合 UnAllocatedCar 中！" << endl;
				}
			}
			cout << endl;
		}*/

		//// 打印：t = 1076时，尚未获得分配的车辆集合 UnAllocatedCar[] 中的车辆信息
		//if (cts >= 1076 && cts <= 1077)
		//{
		//	cout << "当前时间槽为 : cts = " << cts << ", 运行 OTRAP_ALLOC之前，未获得分配的车辆集合 UnAllocatedCar 中的车辆信息，如下：" << endl;
		//	cout << "SeqNum\t" << "Car_Id(Type)\t" << "InitLoc(m)\t" << "V(m/t)\t" << "AT\t" << "DT\t" << "ET\t" << "TaskResourceDemand\t" << "Bid(coefficient)\t" << endl;
		//	int i = 1;
		//	for (auto UACarit = UnAllocatedCar.begin(); UACarit != UnAllocatedCar.end(); UACarit++)
		//	{
		//		cout << i << "\t";
		//		cout << UACarit->id << "(" << UACarit->type << ")\t";
		//		cout << UACarit->InitLoc << "\t";
		//		cout << UACarit->v << "\t";
		//		cout << UACarit->at << "\t";
		//		cout << UACarit->dt << "\t";
		//		cout << UACarit->et << "\t";
		//		cout << "[" << UACarit->TRD->cpu << ", " << UACarit->TRD->gpu << ", " << UACarit->TRD->memory << ", " << UACarit->TRD->disk << "]\t\t";
		//		cout << UACarit->bid << "(" << UACarit->mu << ")" << endl;
		//		i++;
		//	}
		//	cout << endl;
		//}
		...
		OTRAP_ALLOC(cts, ECS, BeAllocatedCar, UnAllocatedCar);
		...
		if (!cts_AllocResult.empty())
		{
			...
			OTRAP_PAY(cts, vEcsInfo_cp, BeAllocatedCar_cp, UnAllocatedCar_cp, cts_AllocResult);
...
void OTRAP_ALLOC(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& BACar, vector<CarInfo>& UACar)
{
	...
	// 检查：计算基站入度后， id = 11499 的车辆的 alloc_flag 是否被置 true;
	/*if (cts >= 719 && cts <= 735)
	{
		cout << "当前时间槽为 : cts = " << cts << endl;
		cout << "检查：计算基站入度后， id = 11499 的车辆的 alloc_flag 是否被置 true." << endl;
		for (auto UACarit = UACar.begin(); UACarit != UACar.end(); UACarit++)
		{
			if (UACarit->id == 11499)
			{
				cout << "车辆id = " << UACarit->id << "  ";
				...
				cout << "TRD = [" << UACarit->TRD->cpu << ", " << UACarit->TRD->gpu << ", " << UACarit->TRD->memory << ", " << UACarit->TRD->disk << "]  ";
				cout << "mu = " << UACarit->mu << "  ";
				cout << "bid = " << UACarit->bid << "  ";
				if (UACarit->alloc_flag == true)
				{
					cout << "alloc_flag = " << UACarit->alloc_flag << endl;
				}
				else
				{
					cout << "计算基站入度后, id = 11499 的车辆的 alloc_flag 没有被置 true！" << endl;
				}
			}
		}
		cout << endl;
	...
	for (int j = 0; j < ECS.size(); j++)
	{
		if (ECS[j].InDegree != 0)	//仅考察入度不为 0 的基站的情况
		{
			for (int i = 0; i < UACar.size(); i++)
			{
				...
				if (UACar[i].alloc_flag == true && UACar[i].ConnectTo == -1)
				{
					...
					if (UACar[i].CM[ecsid - 1][cts - 1] == 1)
					{
						...
						Calc_CarValueDensity(UACar[i], ECS[j]);	//计算 dr, f
			...
			if (!ECS[j].AbleConnectToECS.empty())	// ECS[j].AbleConnectToECS.size() == ECS[j].InDegree
			{
				...
				sort(ECS[j].AbleConnectToECS.begin(), ECS[j].AbleConnectToECS.end(), CarValueDensityDescend);
				...
				for (int i = 0; i < ECS[j].AbleConnectToECS.size(); i++)
				{
					...
					if ((cpuEnough < 0) || (gpuEnough < 0) || (memoryEnough < 0) || (diskEnough < 0))
					{
						...
						// “因基站资源	不足而未被分配的车辆” 被移除 ECS[j].AbleConnectToECS
...
void OTRAP_PAY(int cts, vector<EcsInfo>& ECS, vector<CarInfo>& BACar, vector<CarInfo>& UACar, vector<CarInfo>& cts_AR)
{
	...
	for (int i = 0; i < cts_AR.size(); i++)	// cts_AR = current time slot_Allocate Result
	{
		...
		while (double(fabs(UB_i - LB_i)) >= epsilon)	//还未求得最小的获胜投标(付款)
		{
			...
			OTRAP_ALLOC(cts, Ecs_cp, BACar_cp, UACar_cp);
...
void IsEcsRCEmpty()
{
	...
	for (auto ECSit = vEcsInfo.begin(); ECSit != vEcsInfo.end(); ECSit++)
	{
		if ((ECSit->RC->cpu == 0) && (ECSit->RC->gpu == 0) && (ECSit->RC->memory == 0) && (ECSit->RC->disk == 0))
		{
			flag += 0;
		}
		else
...
{
	if (ecs1.InDegree == ecs2.InDegree)	//若基站的入度相等，按照基站位置升序排列
		return ecs1.loc < ecs2.loc;
	else
		return ecs1.InDegree < ecs2.InDegree;
}

void Calc_CarValueDensity(CarInfo& car, EcsInfo& ecs)
{
	//1.计算车辆 i 的主导资源比例
	double dri_cpu = 0.00, dri_gpu = 0.00, dri_memory = 0.00, dri_disk = 0.00;

	double tmp1 = 0.00, tmp2 = 0.00, tmp3 = 0.00;

	double dr_i = 0.00;

	dri_cpu = (double)car.TRD->cpu / (double)ecs.RC->cpu;
	dri_gpu = (double)car.TRD->gpu / (double)ecs.RC->gpu;
	dri_memory = (double)car.TRD->memory / (double)ecs.RC->memory;
	dri_disk = (double)car.TRD->disk / (double)ecs.RC->disk;

	tmp1 = max(dri_cpu, dri_gpu);
	tmp2 = max(tmp1, dri_memory);
	tmp3 = max(tmp2, dri_disk);

	car.dr = tmp3;	//车辆 i 的主导资源比例 dr_i

	car.f = (double)(car.bid / (car.dr * (double)car.et));	//车辆 i 的资源密度
}

bool CarValueDensityDescend(const CarInfo& car1, const CarInfo& car2)
{
...
void SaveResult()
{
	...
	// 保存算法 OTRAP 的求解结果
	string OTRAP_SolutionFile = "../data/ExperimentalResult/TruthfulnessVerification/OTRAP_Solution_CarInfo_" + to_string(S_CarNum) + "_" + to_string(M_CarNum) + "_" + to_string(L_CarNum) + "-" + to_string(ExeNum) + "_35835(1200).txt";
	...
	//测试：找出运行时间长的原因
	ofs << "(1)OTRAP 准备工作运行时间：PreWork_RunTime = " << PreWork_RunTime << " ms" << endl;
	...
	ofs << "(2)OTRAP 分配函数运行时间：ALLOC_RunTime = " << ALLOC_RunTime << " ms" << endl;
	ofs << "(3)OTRAP 支付函数运行时间：PAY_RunTime = " << PAY_RunTime << " ms" << endl;
	ofs << endl;
...
int main()
{
	...
	//for (; ExeNum <= count_OTRAP; ExeNum++)
	if (ExeNum <= count_OTRAP)		//检查 bug 使用
	{
		cout << "这是第 " << ExeNum << " 次运行 OTRAP 算法 ! " << endl;
		...
		//	cout << endl;
		//}
		/*cout << "读入车辆信息后，检查 id = 11499 的车辆信息，如下：" << endl;
		for (auto vCarit = T_vCarInfo.begin(); vCarit != T_vCarInfo.end(); vCarit++)
		{
			if (vCarit->id == 11499)
			{
				cout << "车辆id = " << vCarit->id << "  ";
				cout << "type = " << vCarit->type << "  ";
				cout << "InitLoc(m) = " << vCarit->InitLoc << "  ";
				cout << "v(m/s) = " << vCarit->v << "  ";
				cout << "at = " << vCarit->at << "  ";
				cout << "dt = " << vCarit->dt << "  ";
				cout << "et = " << vCarit->et << "  ";
				cout << "TRD = [" << vCarit->TRD->cpu << ", " << vCarit->TRD->gpu << ", " << vCarit->TRD->memory << ", " << vCarit->TRD->disk << "]  ";
				cout << "mu = " << vCarit->mu << "  ";
				cout << "bid = " << vCarit->bid << endl;
			}
		}
		cout << endl;*/

		cout << "在时间槽[1, " << T << "]，开始运行 OTRAP 算法！" << endl;
		...
		for (int t = 1; t <= T; t++)
		{
			...
			OTRAP(t, vEcsInfo, T_vCarInfo);
		}

		tEnd = clock();	//计时结束

		cout << "第 " << ExeNum << " 次运行 OTRAP 算法结束！" << endl;
		cout << endl;
		...
		////展示结果
		////运行时间
		//cout << "OTRAP 算法的运行时间为：RunTime = " << RunTime << "ms" << endl;
		////社会福利
		//cout << "整个运行时间[1," << T << "]中，OTRAP算法求解的社会福利为：SocialWelfare = " << TotalSocialWelfare << endl;
		////分配结果
		//cout << "整个运行时间[1," << T << "]中，OTRAP算法求解的分配结果为：AllocResult = " << endl;
		//for (int i = 0; i < AllocResult.size(); i++)
		//{
		...
		/*cout << "运行OTRAP算法后，在时间槽[1,T]完成任务的车辆集合 AccomplishTaskCar = " << endl;
