...
const int count_OTRAP = 10;			// OTRAP 运行次数，设为 10
int ExeNum = 1;						// 第几次运行程序，初始值为 1


// 根据实验要求，每次实验之前都要根据“使用的基站信息”来修改 文件路径 和 文件名
#define EcsInfoFile "../data/ExperimentalResult/TruthfulnessVerification/data/data_EcsInfo_10_0_0.txt"	//基站信息

//根据不用的实验要求，需要修改存放“车辆任务信息”的路径和文件名，如：
//车辆 可连接矩阵 和 其余车辆属性 分开保存（读入的车辆信息小于 20000 条）
#define RS_CarInfoFile_S "../data/ExperimentalResult/TruthfulnessVerification/data/data_RS_CarInfo_S_10000-" + to_string(ExeNum) + "_35835(1200).txt"	//小型车辆任务信息
#define RS_CarInfoFile_S_CM "../data/ExperimentalResult/TruthfulnessVerification/data/data_RS_Car_CM_S_10000-" + to_string(ExeNum) + ".txt"	//小型车辆可连接矩阵

#define RS_CarInfoFile_M "../data/2RandomSampling/exp1/data_RS_CarInfo_M_20000-" + to_string(ExeNum) + ".txt"	//中型车辆任务信息
#define RS_CarInfoFile_M_CM "../data/2RandomSampling/exp1/data_RS_Car_CM_M_20000-" + to_string(ExeNum) + ".txt"	//中型车辆可连接矩阵

#define RS_CarInfoFile_L "../data/2RandomSampling/exp1/data_RS_CarInfo_L_20000-" + to_string(ExeNum) + ".txt"	//大型车辆任务信息
#define RS_CarInfoFile_L_CM "../data/2RandomSampling/exp1/data_RS_Car_CM_L_20000-" + to_string(ExeNum) + ".txt"	//大型车辆可连接矩阵
...
//实验二读入的车辆任务信息比例为：0%, 5%, 10%, 20%, 35%, 50% 
...
const double epsilon = 0.000001;	//定义一个较小的数字，OTRAP_PAY 确定付款使用
